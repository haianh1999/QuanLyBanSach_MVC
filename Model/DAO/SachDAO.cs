...
namespace Model.DAO
{
    public class SachDAO : BaseDAO
    {
        ...
        public List<Sach> ListByCategoryId(string  Ma, ref int totalRecord, int page = 1, int pageSize = 1)
        {
            totalRecord = db_.Saches.Where(x => x.MaTL == Ma).Count();
            var model = db_.Saches.Where(x => x.MaTL == Ma).OrderByDescending(x => x.NgayThem).Skip((page - 1) * pageSize).Take(pageSize).ToList();
            return model;
        }
    }
}
